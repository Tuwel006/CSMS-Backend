...
import { LiveScoreOrchestrator } from '../../../../../realtime/orchestrators/live-score.orchestrator';
...
export class MatchesService {
    ...
    static async getMatchScore(matchId: string, tenant_id: number) {
        ...
        const inningsData = await Promise.all(
            inningsList.map(async (inning) => {
                ...
                const dismissed = MatchesService.buildDismissedBatsmen(batsmen);
                const { items: ballsItems, illegalBallsCount } =
                    MatchesService.buildCurrentOverBalls(currentOverBalls);
                ...
                return {
                    ...
                    batting: {
                        striker: striker ? MatchesService.buildBatsmanSummary(striker) : null,
                        nonStriker: nonStriker ? MatchesService.buildBatsmanSummary(nonStriker) : null
                    },

                    dismissed,

                    bowling: MatchesService.buildBowlersList(bowlers),
    ...
    static async recordBall(matchId: string, ballData: RecordBallDto, tenant_id: number) {
        ...
        setImmediate(() => {
            LiveScoreOrchestrator
                .publishScore(matchId, result.innings_id)
                .catch(err => {
    ...
    static async setBatsman(matchId: string, batsmanData: any, tenant_id: number) {
        ...
        setImmediate(() => {
            LiveScoreOrchestrator
                .publishScore(matchId, match.current_innings_id!)
                .catch(err => {
                    console.error("LiveScore SSE error:", err);
                });
        });
