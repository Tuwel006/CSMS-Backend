...
class TeamSetupService {
    static async setupTeam(data) {
        ...
        try {
            ...
            const playerRepository = queryRunner.manager.getRepository(Player_1.Player);
            ...
            if (match.team_a_id === teamId || match.team_b_id === teamId) {
                throw { status: status_codes_1.HTTP_STATUS.BAD_REQUEST, message: 'Team already assigned to this match' };
            }
            ...
            if (!match.team_a_id) {
                ...
                throw { status: status_codes_1.HTTP_STATUS.BAD_REQUEST, message: 'Both teams already assigned to this match' };
            }
            ...
            for (const playerData of data.players) {
                ...
                if (existingAssignment) {
                    throw { status: status_codes_1.HTTP_STATUS.BAD_REQUEST, message: `Player ${playerData.name} is already assigned to another team in this match` };
                }
    ...
    static async updateTeamSetup(matchId, teamId, data) {
        const queryRunner = db_1.AppDataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            ...
            if (match.team_a_id !== teamId && match.team_b_id !== teamId) {
                throw { status: status_codes_1.HTTP_STATUS.BAD_REQUEST, message: 'Team not assigned to this match' };
            }
            ...
            for (const playerData of data.players) {
                if (!playerData.id) {
                    throw { status: status_codes_1.HTTP_STATUS.BAD_REQUEST, message: 'Player ID is required for update' };
                }
                const matchPlayer = matchPlayerRepository.create({
                    match_id: matchId,
    ...
    static async deleteTeamSetup(matchId, teamId) {
        const queryRunner = db_1.AppDataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
