...
exports.PlayerMatchService = void 0;
const db_1 = require("../../../config/db");
const MatchPlayer_1 = require("../shared/entities/MatchPlayer");
const PlayerStats_1 = require("../shared/entities/PlayerStats");
const Match_1 = require("../shared/entities/Match");
const typeorm_1 = require("typeorm");
class PlayerMatchService {
    constructor() {
        this.matchPlayerRepo = db_1.AppDataSource.getRepository(MatchPlayer_1.MatchPlayer);
        this.playerStatsRepo = db_1.AppDataSource.getRepository(PlayerStats_1.PlayerStats);
    }
    /**
     ...
     */
    async getPlayerMatchSummary(playerId) {
        ...
        return {
            ...
            by_format: {
                T20: allMatches.filter(mp => mp.match.format === Match_1.MatchFormat.T20).length,
                ODI: allMatches.filter(mp => mp.match.format === Match_1.MatchFormat.ODI).length,
                TEST: allMatches.filter(mp => mp.match.format === Match_1.MatchFormat.TEST).length,
            },
            by_status: {
     ...
     */
    async getPlayerRecentMatchesWithStats(playerId, limit = 10) {
        ...
        const stats = await this.playerStatsRepo.find({
            where: {
                player_id: playerId,
                match_id: (0, typeorm_1.In)(matchIds)
            }
        });
     ...
     */
    async getPlayerMatchesByDateRange(playerId, startDate, endDate) {
     ...
     */
    async getPlayerMatchesByTeam(playerId) {
     ...
     */
    async isPlayerInMatch(playerId, matchId) {
     ...
     */
    async getPlayerRoleInMatch(playerId, matchId) {
     ...
     */
    async getMatchPlayers(matchId, teamId) {
     ...
     */
    async getMatchPlaying11(matchId, teamId) {
        return await this.matchPlayerRepo.find({
            where: {
                match_id: matchId,
                team_id: teamId,
     ...
     */
    async getMatchSubstitutes(matchId, teamId) {
        return await this.matchPlayerRepo.find({
            where: {
                match_id: matchId,
                team_id: teamId,
...
exports.PlayerMatchService = PlayerMatchService;
